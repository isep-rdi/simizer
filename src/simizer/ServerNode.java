 ...
 *
 */
public class ServerNode extends Node implements IEventProducer, RequestFinisher {
    ...
    private final EventProducer ep = new EventProducer(); // delegate for IEventProducer
    ...
    public ServerNode(int id, long memSize, int qSize, double cost, final StorageElement se, final Processor p) {
        super(id, null);
    ...
    public ServerNode(int id, long memSize, int nbProc, int qSize, double cost, double nbMips, final StorageElement se) {
        super(id,null);
        this.memory = new Cache(memSize, 0);
        this.memSize = memSize;
        this.disk = se;
    ...
    public int getCapacity() {
        ...
        System.out.println(" total = " + total + " nb " + nb + " memSize "+this.memSize);
        long moy = total / nb;
    ...
    public void onRequestReception(long timestamp, Message m) {
         ...
         * present in cache or disk disk.getRessource and memory.getResource
         * for(int resId: m.getRequest().getResources()) { Resource r =
         * disk.read(resId); }
         *
         * If resource not in cache or disk, send request to next node.
         * nw.getNodeList(); this.nw.getNode(id+1);
         * this.memory.writeToCache(null);
         */
